...
When multiple skills modify the same file, the three-way merge handles the overlap naturally. If Telegram and Discord both modify `src/app.py`, and both skill files include the Telegram changes, those common changes merge cleanly against the base. The result is the base + all skill changes + user customizations.
...
Files like `pyproject.toml`, `docker-compose.yml`, `.env.example`, and generated configs are not code you merge — they're structured data you aggregate. Multiple skills adding dependencies to `pyproject.toml` shouldn't require a three-way text merge. Instead, skills declare their structured requirements in the manifest, and the system applies them programmatically.

**Structured operations are implicit.** If a skill declares `pip_dependencies`, the system handles dependency installation automatically. There is no need for the skill author to add `pip install` to `post_apply`. When multiple skills are applied in sequence, the system batches structured operations: merge all dependency declarations first, write `pyproject.toml` once, run `uv sync` once at the end.
...
- **Dependency version conflicts**: two skills request incompatible version ranges for the same package
...
1. **Automatic where possible**: widen version ranges to find a compatible version, detect and flag port/name collisions
...
skills/
  add-whatsapp/
    ...
    add/                              # New files — copied directly
      ...
      src/channels/whatsapp_config.py
    modify/                           # Modified code files — merged via git merge-file
      src/
        server.py                     # Full file: clean core + whatsapp changes
        server.py.intent.md           # "Adds WhatsApp webhook route and message handler"
        config.py                     # Full file: clean core + whatsapp config options
        config.py.intent.md           # "Adds WhatsApp channel configuration block"
```
...
- Auditable: `diff .g2/base/src/server.py skills/add-whatsapp/modify/src/server.py` shows exactly what the skill changes
...
```markdown
# Intent: server.py modifications
...
test: "pytest tests/test_skills/test_whatsapp.py"
...
file_ops:
  - type: rename
    ...
    to: src/app.py
  - type: delete
    path: src/deprecated/old_handler.py
  - type: move
    from: src/utils/helpers.py
    to: src/lib/helpers.py
```
...
8. Apply structured operations (pip deps, env vars, docker-compose — batched)
9. Run `uv sync` (once, if any structured pip_dependencies exist)
...
When the core renames a file (e.g., `server.py` → `app.py`), skills authored against the old path still reference `server.py` in their `modifies` and `modify/` directories. **Skill packages are never mutated on the user's machine.**
...
path_remap:
  src/server.py: src/app.py
  src/old_config.py: src/config/main.py
```

The system resolves paths at apply time: if a skill targets `src/server.py` and the remap says it's now `src/app.py`, the merge runs against `src/app.py`. The remap is recorded in `state.yaml` so future operations are consistent.
...
```bash
cp skills/add-whatsapp/add/src/channels/whatsapp.py src/channels/whatsapp.py
```
...
```bash
git merge-file src/server.py .g2/base/src/server.py skills/add-whatsapp/modify/src/server.py
```
...
- Merge pip dependencies into `pyproject.toml` (check for version conflicts)
...
The issue is not about conflict marker format — `merge-file` uses filenames as labels (`<<<<<<< current.py`) while `git merge` uses branch names (`<<<<<<< HEAD`), but rerere strips all labels and hashes only the conflict body. The formats are compatible.
...
git merge-file current.py .g2/base/src/file.py skills/add-whatsapp/modify/src/file.py

# 2. If exit code > 0 (conflict), set up rerere adapter:

# Create blob objects for the three versions
base_hash=$(git hash-object -w .g2/base/src/file.py)
ours_hash=$(git hash-object -w skills/previous-skill/modify/src/file.py)  # or the pre-merge current
theirs_hash=$(git hash-object -w skills/add-whatsapp/modify/src/file.py)

# Create unmerged index entries at stages 1 (base), 2 (ours), 3 (theirs)
printf '100644 %s 1\tsrc/file.py\0' "$base_hash" | git update-index --index-info
printf '100644 %s 2\tsrc/file.py\0' "$ours_hash" | git update-index --index-info
printf '100644 %s 3\tsrc/file.py\0' "$theirs_hash" | git update-index --index-info
...
applied_skills:
  - name: telegram
    ...
    file_hashes:
      src/channels/telegram.py: "f627b9cf..."
      tests/test_skills/test_telegram.py: "400116769..."
      src/config.py: "9ae28d1f..."
      src/app.py: "46dbe495..."
      tests/test_skills/test_routing.py: "5e1aede9..."
    structured_outcomes:
      pip_dependencies:
        grammy: ">=1.39.3"
      env_additions:
        - TELEGRAM_BOT_TOKEN
        - TELEGRAM_ONLY
      test: "pytest tests/test_skills/test_telegram.py"

  - name: discord
    version: 1.0.0
    applied_at: 2026-02-17T17:29:37.821Z
    file_hashes:
      src/channels/discord.py: "5d669123..."
      tests/test_skills/test_discord.py: "19e1c6b9..."
      src/config.py: "a0a32df4..."
      src/app.py: "d61e3a9d..."
      tests/test_skills/test_routing.py: "edbacb00..."
    structured_outcomes:
      pip_dependencies:
        discord-py: ">=2.3.0"
      env_additions:
        - DISCORD_BOT_TOKEN
        - DISCORD_ONLY
      test: "pytest tests/test_skills/test_discord.py"
...
- Applied skills use `name` as the key field (not `skill`), matching the `AppliedSkill` dataclass.
- `structured_outcomes` stores the raw manifest values plus the `test` command. Resolved package versions (actual installed versions vs version specifiers) are not yet tracked.
...
```
Detected untracked changes to src/server.py.
...
Update available: 0.5.0 → 0.8.0 (12 commits)
  ...
  Source:  4 files modified (server.py, config.py, ...)
  Skills:  2 new skills added, 1 skill updated
  Config:  pyproject.toml, docker-compose.yml updated
...
```bash
git merge-file src/server.py .g2/base/src/server.py updates/0.5.0-to-0.6.0/files/src/server.py
```
...
merge_pip_dependencies → write pyproject.toml once
uv sync once
