...
By default, DNS resolvers return IPv6 (AAAA) records before IPv4 (A) records. Since our NAT only handles IPv4, applications inside containers will try IPv6 first and fail.

The container image and runner are configured to prefer IPv4. In Python, this is handled by setting the `FORCE_IPV4` environment variable, which the agent runner uses to configure socket preferences. This is set in the `Dockerfile` and passed via `-e` flag in `container_runner.py`.
...
| HTTP works, HTTPS times out | IPv6 DNS resolution | Ensure `FORCE_IPV4=1` is set in container environment |
| `Could not resolve host` | DNS not forwarded | Check bridge100 exists, verify pfctl NAT rules |
| Container hangs after output | Missing `sys.exit(0)` in agent-runner | Rebuild container image |
