...
```
Host (Linux/macOS)                    Container (Linux)
─────────────────────────────────────────────────────────────
src/g2/execution/container_runner.py  container/agent-runner/
    │                                      │
    ...
    ├── data/sessions/{folder}/.claude/ ──> /home/agent/.claude/ (isolated per-group)
    └── (main only) project root ──> /workspace/project
```

**Important:** The container runs as user `agent` with `HOME=/home/agent`. Session files must be mounted to `/home/agent/.claude/` (not `/root/.claude/`) for session resumption to work.
...
```bash
# For development
LOG_LEVEL=debug uv run python -m g2

# For systemd service (Linux), add to the unit file:
...
```
**Fix:** Container must run as non-root user. Check Dockerfile has `USER agent`.
...
The container runs as user `agent` (uid 1000). Check ownership:
```bash
...
All of `/workspace/` and `/app/` should be owned by `agent`.
...
**Root cause:** The SDK looks for sessions at `$HOME/.claude/projects/`. Inside the container, `HOME=/home/agent`, so it looks at `/home/agent/.claude/projects/`.

**Check the mount path:**
```bash
# In container_runner.py, verify mount is to /home/agent/.claude/, NOT /root/.claude/
grep -A3 "claude.*session" src/g2/execution/container_runner.py
```

**Verify sessions are accessible:**
```bash
docker run --rm --entrypoint /bin/bash \
  -v $(pwd)/data/sessions/main/.claude:/home/agent/.claude \
  g2-agent:latest -c '
echo "HOME=$HOME"
...
**Fix:** Ensure `container_runner.py` mounts to `/home/agent/.claude/`:
```python
mounts.append(Mount(
    host_path=claude_dir,
...
```bash
# No build step needed for host app — Python runs directly
...
- Container HOME: `/home/agent`
- Mount target: `/home/agent/.claude/` (NOT `/root/.claude/`)
...
grep -q "/home/agent/.claude" src/g2/execution/container_runner.py 2>/dev/null && echo "OK" || echo "WRONG - should mount to /home/agent/.claude/, not /root/.claude/"
