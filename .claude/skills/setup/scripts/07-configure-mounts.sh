...
JSONEOF
  ...
  # Validate JSON
  if ! echo "$INPUT" | uv run python -c "import json, sys; json.load(sys.stdin)" 2>/dev/null; then
...
EOF
  ...
  # Extract values
  ALLOWED_ROOTS=$(uv run python -c "import json; d=json.load(open('$CONFIG_FILE')); print(len(d.get('allowedRoots',[])))" 2>/dev/null || echo "0")
  NON_MAIN_READ_ONLY=$(uv run python -c "import json; d=json.load(open('$CONFIG_FILE')); print('false' if d.get('nonMainReadOnly') is False else 'true')" 2>/dev/null || echo "true")
fi
