...
get_phone_number() {
  if [ -f "$PROJECT_ROOT/store/auth/creds.json" ]; then
    uv run python -c "
import json
c = json.load(open('store/auth/creds.json'))
if 'me' in c and 'id' in c['me']:
    print(c['me']['id'].split(':')[0].split('@')[0], end='')
" 2>/dev/null || true
  fi
}
...
case "$METHOD" in
  ...
  qr-browser)
    ...
    uv run python -m g2 auth >> "$LOG_FILE" 2>&1 &
    AUTH_PID=$!
...
with open('store/qr-auth.html', 'w') as f:
    f.write(template.replace('{{QR_SVG}}', svg))
" >> "$LOG_FILE" 2>&1

    # Open in browser
    if command -v xdg-open >/dev/null 2>&1; then
      xdg-open "$PROJECT_ROOT/store/qr-auth.html"
      ...
      log "WARNING: no browser open command found"
    fi
...
EOF
    ...
    uv run python -m g2 auth --pairing-code --phone "$PHONE" >> "$LOG_FILE" 2>&1 &
    AUTH_PID=$!
    log "Auth process started (PID $AUTH_PID)"
