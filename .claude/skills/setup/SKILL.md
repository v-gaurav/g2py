...
Python 3.12+ is missing or too old. Ask the user if they'd like you to install it. Offer options based on platform:

- macOS: `brew install python@3.12` (if brew available)
- Linux: `sudo apt-get install -y python3.12 python3.12-venv` (Debian/Ubuntu) or `sudo dnf install -y python3.12` (Fedora)

**If UV_OK=false:**

uv is missing. Install it:
```bash
curl -LsSf https://astral.sh/uv/install.sh | sh
```

After installing, re-run the environment check to confirm.
...
1. Delete `.venv` and re-run the script (fresh virtual environment)
2. If permission errors: suggest running with corrected permissions
3. If specific package fails to build (native modules): install build tools (`xcode-select --install` on macOS, `build-essential` on Linux), then retry
...
**If APPLE_CONTAINER=installed** (macOS only): Ask the user which runtime they'd like to use — Docker (default, cross-platform) or Apple Container (native macOS).
...
- **QR terminal:** Run `./.claude/skills/setup/scripts/04-auth-whatsapp.sh --method qr-terminal`. Tell user to run `cd PROJECT_PATH && uv run python -m g2 auth` in another terminal. Wait for confirmation.
...
First, determine the phone number situation. Get the bot's WhatsApp number from `store/auth/` using:
`uv run python -c "import json; c=json.load(open('store/auth/creds.json')); print(c['me']['id'].split(':')[0].split('@')[0])"`
...
2. **If SYNC=failed:** Read `logs/setup.log` for the sync output. Common causes: WhatsApp auth expired (re-run step 5), connection timeout (re-run sync script with longer timeout).
3. **If GROUPS_IN_DB=0:** Check `logs/setup.log` for details.
...
Tell user how to grant a group access: add `containerConfig.additionalMounts` to their entry in the database.

## 10. Start Service

If the service is already running (check `systemctl --user is-active g2` on Linux or `launchctl list | grep g2` on macOS), stop it first — then proceed with a clean install.

Run `./.claude/skills/setup/scripts/08-setup-service.sh` and parse the status block.

**If SERVICE_LOADED=false:**
- Read `logs/setup.log` for the error.
- On macOS: check `launchctl list | grep g2` to see if it's loaded with an error status. If the PID column is `-` and the status column is non-zero, the service is crashing. Read `logs/g2.error.log` for the crash reason and fix it (common: missing .env, missing auth, wrong Python/uv path).
...
- SERVICE=stopped → restart: `systemctl --user restart g2` (Linux) or `launchctl kickstart -k gui/$(id -u)/com.g2` (macOS). Re-check.
...
**Service not starting:** Check `logs/g2.error.log`. Common causes: wrong Python/uv path in service config, missing `.env` (re-run step 4), missing WhatsApp auth (re-run step 5).
...
**No response to messages:** Verify the trigger pattern matches. Main channel and personal/solo chats don't need a prefix. Check the registered groups in the database: `uv run python -c "import aiosqlite, asyncio; asyncio.run((lambda: None)())"` or use `sqlite3 store/messages.db "SELECT * FROM registered_groups"`. Check `logs/g2.log`.
...
**WhatsApp disconnected:** Run `uv run python -m g2 auth` to re-authenticate, then restart the service.

**Unload service:** `systemctl --user stop g2` (Linux) or `launchctl unload ~/Library/LaunchAgents/com.g2.plist` (macOS)
